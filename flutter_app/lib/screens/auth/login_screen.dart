...
class _LoginScreenState extends State<LoginScreen> {
  ...
  void _handleLogin() async {
    if (_formKey.currentState!.validate()) {
      ...
      // Remove processing message - no delay needed
      ...
      try {
        ...
        if (mounted) {
          if (result['success']) {
            // Login successful - no success message needed
            // ScaffoldMessenger.of(context).showSnackBar(
            //   SnackBar(
            ...
            // Debug logging
            developer.log(
              'Login result: ${result.toString()}',
              name: 'LoginScreen',
            );

            // Navigate immediately - no delay for admin users (no success message delay)
            if (result['user'] != null && result['user'].isAdmin == true) {
              developer.log(
                'User is admin, navigating to admin dashboard immediately',
                name: 'LoginScreen',
              );
              Navigator.pushReplacementNamed(context, '/admin');
            } else {
              developer.log(
                'User is not admin, navigating to admin dashboard immediately (should not happen for home page users)',
                name: 'LoginScreen',
              );
              Navigator.pushReplacementNamed(context, '/admin');
            }
